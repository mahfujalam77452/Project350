...
function EventsList() {
    ...
    const renderEventContent = () => {
        ...
        if (events.length === 0) {
            return (
                <div className="text-center text-gray-500 dark:text-gray-400 p-6 border border-dashed rounded">
                    No events found.
                </div>
            );
        }
        ...
        return (
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {events.map((event) => {
                    ...
                    return (
                        <div key={event.id || event._id} className="panel card bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden flex flex-col hover:shadow-lg transition-shadow duration-300">
                            {/* Card Image */}
                            <img 
                            ...
                            <div className="p-4 flex-grow">
                                <h2 className="text-lg font-semibold mb-2 text-gray-800 dark:text-white truncate" title={event.title}>{event.title || 'Untitled Event'}</h2>
                                <p className="text-gray-600 dark:text-gray-300 text-sm mb-4 line-clamp-3">{event.description || 'No description available.'}</p>
                                    ...
                                    {event.clubName && (
                                        <div className="flex items-center">
                                            <BuildingLibraryIcon className="mr-2 flex-shrink-0 w-5 h-5 text-blue-500" />
                            ...
                            {/* Card Footer - Actions */}
                            <div className="p-3 bg-gray-50 dark:bg-gray-700 border-t border-gray-200 dark:border-gray-600 flex justify-end space-x-2">
                                <button 
                                    onClick={() => navigate(`/event/edit/${event.id || event._id}`)} 
                                    className="btn btn-sm btn-outline-primary p-1 rounded-full hover:bg-primary-light"
                                    title="Edit Event"
                                >
                                    <FaEdit size={16}/>
                                </button>
                                <button 
                                    ...
                                    className="btn btn-sm btn-outline-danger p-1 rounded-full hover:bg-danger-light"
                                    title="Delete Event"
                                >
                                    <FaTrash size={16}/>
                                </button>
